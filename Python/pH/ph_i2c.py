...
def main():
    ...
    print(" where xx.x is longer than the %0.2f second timeout." %
    ...
    while True:
        ...
        # continuous polling command automatically polls the board
            ...
            delay_time = float(string.split(input_command, ',')[1])

            # check for polling time being too short, change it to the minimum timeout if too short
            if delay_time < AtlasI2C.long_timeout:
                print("Polling time is shorter than timeout, setting polling time to %0.2f" %
                      AtlasI2C.long_timeout)
                delay_time = AtlasI2C.long_timeout

            # get the information of the board you're polling
            info = string.split(device.query("I"), ",")[1]
            print("Polling %s sensor every %0.2f seconds, press ctrl-c to stop polling" %
                  (info, delay_time))
